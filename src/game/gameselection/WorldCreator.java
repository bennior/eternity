...
public class WorldCreator {
	...
	private void creating(int x, int cxPos, int cyPos) {
		...
		if((cxPos >= px * Panel.GAME_SCALE_WIDTH && cxPos <= (px + pwidth) * Panel.GAME_SCALE_WIDTH && cyPos <= py * Panel.GAME_SCALE_HEIGHT && cyPos >= (py - pheight) * Panel.GAME_SCALE_HEIGHT) || KeyInputs.keyCode == KeyEvent.VK_ENTER) {
			if(!GameSelection.ifWorldExists(textField.getInput()) && !textField.getInput().equals("")) {
				//set up new world
				creating = false;
